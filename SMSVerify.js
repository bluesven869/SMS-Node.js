...
SMSVerify.prototype.request = function(phone) {
    ...
    this.twilioClient.messages.create({
        ...
        /*
        Phone.findOne({ phone: phone }, function(err, oldPhone){
            if (err) return ;
            if (!oldPhone) return;             
            oldPhone.remove(function(){
                return;
            });
        });
        var newPhone = new Phone();        
        newPhone.smsCode = smsMessage;
        newPhone.phoneNumber = phone;
        newPhone.createdTimestamp = Date.now;
        newPhone.isVerified = false;
        newPhone.save();*/
    })
