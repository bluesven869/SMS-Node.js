...
// Connect to database
//mongoose.connect("mongodb://localhost:27017/sms_verification");
...
app.post('/api/request', jsonBodyParser, function(request, response) {
    ...
    if (phone == null) {
        ...
        response.status(500).send('phone is required.');
        return;
    }
...
app.post('/api/verify', jsonBodyParser, function(request, response) {
    ...
    if (phone == null || smsMessage == null) {
        ...
        response.send(500, 'sms_message parameters are required');
        return;
    }
...
app.post('/api/reset', jsonBodyParser, function(request, response) {
    ...
    if (phone == null) {
        // send an error saying that all parameters are required
        response.send(500, 'The phone parameter is required');
        return;
    }
